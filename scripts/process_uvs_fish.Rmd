...
fish_obs <- caribe_obs %>% 
  ...
  left_join(uvs_meta %>% 
              mutate(diver = case_when(diver == "Luis Chasqui" ~ "LCV",
                                       ...
                                       TRUE ~ diver)) %>% 
              distinct(ps_station_id, diver, location, sublocation, 
                       depth_m, ps_sample_id, habitat)) %>%
...
fish_obs <- fish_obs %>% 
  ...
  select(ps_station_id, ps_sample_id, ps_transect_id, order, family, taxon_valid_name, n_ind, avg_length,
         min_length, max_length, diver, transect, depth_m, depth_strata, location, sublocation,
         habitat, a, b, max_length_tl) %>% 
...
biomass_by_taxa_and_transect %>% 
  left_join(fish_transects %>% 
              select(ps_sample_id, ps_transect_id)) %>% 
  select(ps_sample_id, ps_transect_id, everything()) %>% 
...
station_summary <- transect_summary %>% 
  left_join(uvs_meta %>% 
              dplyr::select(sublocation, ps_sample_id, ps_station_id, habitat = habitat_sp)) %>% 
...
biomass_by_taxa_and_station %>% 
  ...
  summarize(across(c(n_ind, ind_m2, gr_m2), mean)) %>% 
  ungroup() %>% 
  filter(n_ind > 0) %>% 
  group_by(sublocation) %>% 
  slice_max(n = 10, order_by = gr_m2)
...
serr_map <- station_summary %>% 
  ...
  geom_sf(aes(col = habitat, size = gr_m2), alpha = 0.85)+
  geom_sf(aes(size = gr_m2), color = alpha("white",.05))+
...
bn_map <- station_summary %>% 
  ...
  geom_sf(aes(col = habitat, size = gr_m2), alpha = 0.8)+
...
adonis2(transect_summary %>% 
          filter(location == "Golfo de Tribuga") %>% 
          select(gr_m2) ~  depth_strata + diver + sublocation, 
        data = fish_transects %>% 
          filter(location == "Golfo de Tribuga"),
