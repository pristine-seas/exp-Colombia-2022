...
cover_by_station_and_taxa <- clean_lpi |>
  ...
  group_by(ps_station_id) |>
  mutate(pct_cover = 100 * contacts / sum(contacts)) |>
  ungroup() |> 
  filter(contacts > 0)
```
...
cover_by_station_and_group |> 
  ...
  scale_fill_manual(values = c(ps_colors("functional_groups"), "seagrass" = "green"))+
...
plot_df <- cover_by_station_and_taxa |> 
  select(ps_station_id, morphotaxon, accepted_name, functional_group, accepted_aphia_id, rank, pct_cover, contacts) |> 
...
```{r}
stations_tbl_colnames <- tbl(bq_connection, "uvs.lpi_stations") |> 
  filter(!is.na(exp_id)) |> 
  head() |> collect() |> colnames()

# figure out which are missing
missing_cols <- setdiff(stations_tbl_colnames, names(station_summary))

# create them as NA
station_summary <- station_summary |>
  dplyr::mutate(!!!rlang::set_names(rep(list(NA), length(missing_cols)), missing_cols)) |>
  # enforce order
  dplyr::relocate(all_of(stations_tbl_colnames)) |> 
  select(all_of(stations_tbl_colnames)) 

write_csv(station_summary, file.path(exp_path, "data/primary/output/benthos/lpi_station_summary.csv"))
```

```{r}
cover_tbl_colnames <- tbl(bq_connection, "uvs.lpi_cover_by_taxa") |> 
  colnames()

# figure out which are missing
cover_missing_cols <- setdiff(cover_tbl_colnames, names(cover_by_station_and_taxa))

write_csv(cover_by_station_and_taxa, file.path(exp_path, "data/primary/output/benthos/lpi_cover_by_station_and_taxa.csv"))
```

```{r}
contact_tbl_colnames <- tbl(bq_connection, "uvs.lpi_contacts") |> 
  colnames()

clean_lpi <- clean_lpi |> 
  mutate(notes = NA_character_) |> 
  select(all_of(contact_tbl_colnames))
