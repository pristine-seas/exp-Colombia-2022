...
exp_config <- list(exp_id      = "COL_2022",
                   date_bounds = c(as.Date("2022-03-01"), as.Date("2022-05-31")),
                   lat_min     = -12.3, 
                   lat_max     = -5,  
                   lon_min     = 155, 
                   lon_max     = 170)  
...
```{r}
uvs_meta_pacifico <- readxl::read_excel(file.path(exp_path, "data", "primary","raw","fish","tribuga_fish_metadata.xlsx")) |> 
  janitor::clean_names() |> 
  mutate(region = "Golfo de Tribugá") |> 
  rename(subregion = sublocation)

uvs_meta_caribe <- readxl::read_excel(file.path(exp_path, "data", "primary","raw","fish","SEA_fish_metadata.xlsx")) |>  
  janitor::clean_names() |> 
  mutate(location = str_to_title(str_replace_all(location, "_", " "))) |> 
  rename(region = location) |> 
  mutate(subregion = region)

uvs_sites <- uvs_meta_pacifico |> 
  bind_rows(uvs_meta_caribe) 

uvs_sites$subregion[uvs_sites$site_name == "La Chola"] <- "PNN Utria"

uvs_sites$subregion[uvs_sites$site_name == "Don Diego"]<- "PNN Utria"

uvs_sites <- uvs_sites |> 
  rename(latitude   = lat,
         longitude  = lon) |> 
  mutate(ps_site_id = paste0("COL_2022_uvs_", formatC(row_number(), width = 3, flag = 0)),
         exp_id     = exp_id,
         method     = "uvs",
         ...
         locality   = site_name,
         exposure   = case_when(str_detect(habitat, "lagoon") ~ "lagoon", 
                                str_detect(ps_site_id, "uvs_028|uvs_031|uvs_035") ~ "channel",
                                str_detect(ps_site_id, "uvs_029|uvs_045") ~ "lagoon",
                                str_detect(ps_site_id, "uvs_007") ~ "sheltered",
                                subregion == "Cabo Corrientes" ~ "exposed",
                                TRUE ~ "unknown"),
         in_mpa     = if_else(region == "Golfo de Tribugá", TRUE, FALSE),
         mpa_notes  = case_when(subregion == "PNN Utria" ~ "PNN Utria",
                                region    == "Golfo de Tribugá" ~ "DRMI Golfo de Tribugá - Cabo Corrientes",
                                TRUE ~ NA_character_),
         notes      = NA_character_) |> 
...
uvs_sites <- uvs_sites |> 
  mutate(habitat = case_when(str_detect(ps_site_id, "uvs_037|uvs_039|uvs_043|uvs_044|uvs_046") ~ "fringing_reef",
                             str_detect(ps_site_id, "uvs_029") ~ "patch_reef",
                             habitat == "seagrass beds" ~ "seagrass",
                             habitat == "lagoon patch reef" ~ "patch_reef",
                             habitat == "forereef, mixed coral, sponges, gorgonians" ~ "fore_reef",
                             habitat == "offshore pinnacle" ~ "pinnacle_reef",
                             habitat == "deep platform, sponges and gorgonians" ~ "reef_pavement",
                             habitat == "nearshore coral reef" ~ "patch_reef",
                             habitat %in% c("nearshore rocky reef", "rocky point", "offshore islands rocky reef", "offshore islet", "offshore rocks") ~ "rocky_reef",
                             TRUE ~ habitat)) 
...
plot_uvs_sites_map(region = "Golfo de Tribugá", subregion = NULL,
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Tribuga.pdf"))

plot_uvs_sites_map(region = "Serranilla", subregion = NULL,
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Serranilla.pdf"))

plot_uvs_sites_map(region = "Bajo Nuevo", subregion = NULL,
                   save = TRUE, path = file.path(exp_path, "figures/site_maps/", "uvs_Bajo_Nuevo.pdf"))
```
