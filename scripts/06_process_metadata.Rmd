...
pcam_meta <- pcam_meta %>% 
  ...
  mutate(date = lubridate::ymd(date),
         ...
         sublocation = case_when(date <= ymd("2022-03-06") ~ "Navigator bank", 
                                 ...
                                 TRUE ~ "Serranilla"),
         depth_m = 10) %>% 
  dplyr::select(ps_station_id, ps_sample_id, method, date, location, sublocation, time_in, lon_in = long_in, lat_in, depth_m, time_out, 
...
invert_contacts <- bind_rows(invert_contacts_pacifico,
  ...
  transmute(ps_station_id = paste0("COL_uvs_", station_number),
            ...
            sampling_method = "Point contacts") %>% 
...
vagile_inverts_obs <- vagile_inverts_obs %>% 
  ...
  transmute(ps_station_id = paste0("COL_uvs_", station_number),
            ...
            sampling_method = "Vagile inverts survey") %>% 
  distinct(ps_station_id, sampling_method, depth_m, diver) 
```
...
uvs_meta <- uvs_meta %>% 
  left_join(uvs_methods_meta, by = "ps_station_id") %>% 
...
bruvs_meta <- bruvs_meta %>% 
  transmute(ps_station_id = paste0("COL_bruvs_", 
            ...
            depth = ifelse(str_detect(depth, "ft"), 
...
```{r}
#shrd_vars <- c("ps_station_id", "method", "date", "location", "sublocation", "time")

exp_col_meta <- bind_rows(
  tags_sets_meta %>% 
    filter(set_check_number == "Set") %>% 
    dplyr::select(ps_station_id, method, date, location, sublocation, 
                  time = time_in, lat, lon, team_member = operator) %>% 
    distinct(),
  bruvs_meta %>% 
    distinct(ps_station_id, method, date, location, sublocation,
             time = local_time, lat, lon, depth_m = depth, team_member = operator),
  ysi_meta %>% 
    distinct(ps_station_id, method, date, location, sublocation, 
             time, lat, lon,  depth_m = max_depth_m, team_member = operator),
  uvs_meta %>% 
    distinct(ps_station_id, method, date, location, sublocation, 
             time = local_time, lat, lon,  depth_m, team_member = diver),
  birds_meta %>% 
    distinct(ps_station_id, method, date, location, sublocation, 
             time = time_start, lat = lat_start, lon = long_start, team_member = observer),
  edna_meta %>% 
    distinct(ps_station_id, method = sampling_method, date, location, sublocation, 
             time = collection_time, lat, lon,  depth_m, team_member = operator),
  meso_meta %>% 
    distinct(ps_station_id, method, date, location, sublocation, 
             time = time_in, lat = lat_in, lon = lon_in,  depth_m, team_member = operator),
  pcam_meta %>% 
    distinct(ps_station_id, method, date, location, sublocation, 
             time = time_in, lat = lat_in, lon = lon_in, team_member = operator, depth_m),
  sub_meta %>% 
    distinct(ps_station_id, method, date, location, sublocation, 
  ...
  dscm_meta %>% 
    distinct(ps_station_id, method, date, location, sublocation, 
             time = time_in, lat = lat_in, lon = lon_in, depth_m = max_depth_m, 
...
```{r}
exp_col_meta %>% 
  filter(location == "Caribbean", !method %in% c("YSI probe", "Coral presence", "Point contacts", "Vagile inverts survey")) %>% 
