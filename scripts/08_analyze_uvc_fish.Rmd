...
uvc_meta <- read_csv(file.path(ps_exp_path, 
  ...
  filter(method == "Fish visual census", location == "Caribbean") %>% 
...
fish_data_by_station <- fish_data_by_transect %>% 
  ...
  ungroup() %>% 
  mutate_if(is.numeric, round, 2) %>% 
  select(sublocation, habitat, ps_station_id, family, taxon_sci_name, 
...
serr_map <- station_summary %>% 
  ...
  theme_light()+
  labs(col = "Habitat", size = bquote('Biomass density '(gr/m^2)))+
...
ggsave(serr_map,
       filename = file.path(ps_exp_path, "figures", "serranilla_fish_biomass_map.png"), 
       dpi = 300)
```
...
ggsave(bn_map,
       filename = file.path(ps_exp_path, "figures", "bajo_nuevo_fish_biomass_map.png"), 
...
community_pcoa_plots[[1]] %>% 
  ggsave(filename = file.path(ps_exp_path, "figures", "uvc_caribe_fish_biomas_pcoa_by_habitat.png"), 
