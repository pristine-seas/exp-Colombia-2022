...
accum_curve_by_atoll <- ggiNEXT(ac_curve_by_atoll)+
  ...
  scale_color_manual(values = c("#C59B76", "#78BC61"))+
  scale_fill_manual(values = c("#C59B76", "#78BC61"))
```
...
acc_data_by_habitat_serr <- fish_data_by_transect %>%
  ...
  group_by(habitat) %>% 
  group_split(keep = F)  %>% 
  set_names(c("A. frontal", "A. periferico", "A. lagunar", "pastos")) %>% 
  map(.f = ~column_to_rownames(.x, "taxon_sci_name")) %>% 
  map(.f = ~select_if(.x, ~ !is.numeric(.) || sum(.) != 0))%>% 
  map(.f = ~filter(.x, rowSums(.x)>0))
...
accum_curve_serra <- ggiNEXT(ac_curve_by_hab_serra)+
  ...
  theme(legend.position = "right")+
  guides(linetype = "none")+
  PristineSeasR::scale_color_pristine_seas()+
...
acc_data_by_habitat_bn <- fish_data_by_transect %>%
  ...
  group_by(habitat) %>% 
  group_split(keep = F)  %>% 
  set_names(c("M. profunda", "A. frontal", "A. lagunar")) %>% 
  map(.f = ~column_to_rownames(.x, "taxon_sci_name")) %>% 
  map(.f = ~select_if(.x, ~ !is.numeric(.) || sum(.) != 0)) %>% 
  map(.f = ~filter(.x, rowSums(.x)>0))
...
ggsave(acc_curves, filename = file.path(ps_exp_path, "figures", "uvc_caribe_fish_acc_curves.png"), 
       dpi = 300, width = 8, height = 10)


ggsave('test.png', test, width = 8, height = 8)
```
