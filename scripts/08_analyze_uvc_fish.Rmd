...
serr_map_spanish <- serr_map +
  paletteer::scale_color_paletteer_d("RColorBrewer::Set3", 
                                     labels = c("arrecife frontal", "arrecife periferico","arrecife lagunar", "pastos marinos"))+
...
bn_map_spanish <- bn_map+
  paletteer::scale_color_paletteer_d("RColorBrewer::Set3", 
                                     labels = c("meseta profunda", "arrecife frontal","arrecife lagunar"))+
  labs(col = "Habitat", size = bquote('Biomasa '(g/m^2)))
...
univariate_plots_by_habitat_SP <- univariate_plots_by_habitat +
  paletteer::scale_fill_paletteer_d("ggsci::default_jama", 
                                    labels = c("meseta profunda", "arrecife frontal",
                                               "arrecife periferico", "arrecife lagunar", 
                                               "pastos marinos")) 
...
```{r}
fish_data_by_transect_wide <- fish_data_by_transect %>%
   dplyr::select(sublocation, transect_id, habitat, ps_taxon_code, n_ind) %>%
  mutate(n_ind = if_else(n_ind > 0, 1, 0)) %>% 
   pivot_wider(names_from = ps_taxon_code, values_from = n_ind, values_fill = 0)
  
acc_curves <- BiodiversityR::accumcomp(fish_data_by_transect_wide %>% 
                                         select(-sublocation, -transect_id, -habitat),
                                       y = fish_data_by_transect_wide %>% 
                                         select(sublocation, habitat),
                                       factor='sublocation', 
                                       method='exact', conditioned = FALSE, plotit = FALSE) %>% 
  BiodiversityR::accumcomp.long(ci = NA, label.freq = 4)

bajo_nuevo_acc_curves <- BiodiversityR::accumcomp(fish_data_by_transect_wide %>% 
                                                    filter(sublocation == "Bajo Nuevo") %>% 
                                                    select(-sublocation, -transect_id, -habitat),
                                                  y = fish_data_by_transect_wide %>% 
                                                    filter(sublocation == "Bajo Nuevo") %>% 
...
serranilla_acc_curves <- BiodiversityR::accumcomp(fish_data_by_transect_wide %>% 
                                                    filter(sublocation == "Serranilla") %>% 
                                                    select(-sublocation, -transect_id, -habitat),
                                                  y = fish_data_by_transect_wide %>% 
                                                    filter(sublocation == "Serranilla") %>% 
...
acc_curve_by_location <- acc_curves %>% 
  ...
  theme(axis.title = ggplot2::element_text(family = "Helvetica", size = 18, color = "#222222"))+
  labs(x = "Numero de transectos", y = "Diversidad de especies", colour = "", shape = "")

acc_curve_serranilla <- serranilla_acc_curves %>% 
  ggplot(aes(x = Sites, y = Richness, ymax = UPR, ymin = LWR)) + 
  scale_x_continuous(expand=c(0, 1), sec.axis = dup_axis(labels=NULL, name=NULL)) +
  scale_y_continuous(sec.axis = dup_axis(labels=NULL, name=NULL)) +
  geom_line(aes(colour=Grouping), size=2) +
  geom_point(data=subset(serranilla_acc_curves, labelit==TRUE), 
             aes(colour=Grouping, shape=Grouping), size=5) +
  geom_ribbon(aes(colour=Grouping), alpha=0.2, show.legend=FALSE)+
  bbplot::bbc_style()+
  paletteer::scale_color_paletteer_d("ggsci::default_jama")+
...
acc_curve_bajo_nuevo <- bajo_nuevo_acc_curves %>% 
  ggplot(aes(x = Sites, y = Richness, ymax = UPR, ymin = LWR)) + 
  scale_x_continuous(expand=c(0, 1), sec.axis = dup_axis(labels=NULL, name=NULL)) +
  scale_y_continuous(sec.axis = dup_axis(labels=NULL, name=NULL)) +
  geom_line(aes(colour=Grouping), size=2) +
  geom_point(data=subset(bajo_nuevo_acc_curves, labelit==TRUE), 
             aes(colour=Grouping, shape=Grouping), size=5) +
  geom_ribbon(aes(colour=Grouping), alpha=0.2, show.legend=FALSE)+
  bbplot::bbc_style()+
  paletteer::scale_color_paletteer_d("ggsci::default_jama")+
...
pcoa_by_hab <- community_pcoa_plots[[1]] +
  labs(color = "Habitat") + 
  paletteer::scale_color_paletteer_d("ggsci::default_jama", 
                                     labels = c("arrecife frontal", "arrecife periferico","arrecife lagunar")) 
  
library(patchwork)

pcoa_plots <- pcoa_by_location/pcoa_by_hab 
  
ggsave(pcoa_plots, filename = file.path(ps_exp_path, "figures", "uvc_caribe_fish_pcoa_plots.png"), 
