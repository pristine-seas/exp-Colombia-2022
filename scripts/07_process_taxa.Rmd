...
dscm_meta <- read_csv(file.path(ps_exp_path, "data", "primary", "processed","metadata" ,"dscm_meta.csv"))
...
pcam_taxa_pacific <- pcam_taxa_pacific %>% 
  ...
  bind_cols(pcam_taxa_pacific %>% 
              ...
              mutate(taxon = str_to_lower(str_trim(str_squish(taxon)))) %>% 
              select(taxon))
```
...
bruvs_taxa <- bruvs_maxN %>% 
  ...
  mutate(location = "Caribbean",
         ...
         taxon = case_when(taxon == "C. acronotus" ~ "Carcharhinus acronotus",
                           ...
                           taxon == "H. americanus" ~ "Homarus americanus",
...
bn_taxa <- readxl::read_xlsx(file.path(ps_exp_path, "data/primary/raw",
  ...
  mutate(location = "Caribbean",
         method = "sub") %>% 
  distinct(location, method, taxon = taxa)
```
...
clean_master_taxa <- clean_master_taxa %>% 
  coalesce_join(clean_taxa_gbif %>% 
                  select(taxon_clean = original_search, gbif_sci_name = taxon_valid_name,
                         taxon_rank, 
                         taxon_id,
                         taxon_authority,
                         kingdom, phylum, class, order, family, genus) %>%
                  distinct(), by = "taxon_clean") 

clean_master_taxa %>% 
  filter(worms_sci_name != gbif_sci_name | is.na(worms_sci_name)) %>% 
  distinct(taxon, gbif_sci_name,worms_sci_name, taxon_clean)
...
clean_master_taxa <- read_csv("temp.csv")
```

# IUCN

```{r}
iucn_db <- read_csv(file.path(ps_data_path, "iucn-redlist-marine-species",  "joined_and_resolved_taxa.csv"))
...
clean_master_taxa %>% 
  filter(is.na(a) & taxon_rank == "Species", class %in% c("Actinopteri", "Actinopterygii", "Elasmobranchii")) %>% 
  distinct()
```
```{r}
...
clean_master_taxa <- clean_master_taxa %>% 
  ...
  mutate(method_ab = if_else(is.na(method_ab) & !is.na(a), "Imputed at genus level", method_ab)) 
```
